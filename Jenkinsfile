				...
				script {
				    ...
				    docker.withRegistry('http://nexus:8081', 'Nexus') {
				        if(env.BRANCH_NAME != "production") {
				            app = docker.build("docker-releases/people_backend_${env.BRANCH_NAME}:$TAG", "--build-arg JAR_FILE=build/libs/people-backend-${TAG}.jar .")
                            app.push("$TAG")
				        } else {
                            app = docker.build("docker-releases/people_backend:$TAG", "--build-arg JAR_FILE=build/libs/people-backend-${TAG}.jar .")
                            app.push("$TAG")
                        }
                    }
				}
